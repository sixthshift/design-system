...
- [ ] **No colour named in the `.tsx`.** Every colour reads a `--{component}-*` token; the mapping lives in `src/components/<Component>/<component>.recipe.css` (imported by `src/theming/tailwind.css`), inside `@layer components`, selected by `data-variant`/`data-intent`.
- [ ] Recipe file created **and** imported by `src/theming/tailwind.css`; `bun run check:recipes` passes.
