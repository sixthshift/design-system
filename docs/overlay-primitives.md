...
4. **`data-state` drives the animation.** The floating node is stamped with `data-state={state}`, and the `animate-*` classes are gated on that state. The classes map to keyframes declared in `tailwind.config.ts:197`.
...
Animations are defined once. `animate-fade-in/out` (Modal overlay + desktop modal, Toast), `animate-slide-up-in/out` (mobile modal sheet), and `animate-slide-{right,left}-in/out` (Sheet by side) all resolve to keyframes in `tailwind.config.ts:197`, each `200ms ease-out forwards`. A global `prefers-reduced-motion` rule collapses every animation to `0.01ms` (`tailwind.config.ts:249`), so overlays inherit reduced-motion behavior for free.
